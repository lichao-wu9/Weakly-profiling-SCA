import numpy as np
import h5py
from scipy import stats

AES_Sbox = np.array([
    
])

AES_Sbox_inv = np.array([
    
])

def labelize(plaintexts, keys, byte):
    if len(keys) != len(plaintexts):
        return AES_Sbox[plaintexts[:, byte] ^ keys[byte]]
    else:
        return AES_Sbox[plaintexts[:, byte] ^ keys[:, byte]]

def addGussianNoise(traces, noise_level):
    print('Add Gussian noise: ', noise_level)
    if noise_level == 0:
        return traces
    else:
        return traces + np.random.normal(0, noise_level, size=np.shape(traces))

def addDesync(traces, desync_level):
    print('Add desync noise...')
    traces_length = len(traces[0])
    if desync_level == 0:
        return traces
    else:
        output_traces = np.zeros((len(traces), traces_length-desync_level))
        for idx in range(len(traces)):
            if(idx % 2000 == 0):
                print(str(idx) + '/' + str(len(traces)))
            rand = np.random.randint(low=0, high=desync_level)
            output_traces[idx] = traces[idx][rand:rand+traces_length-desync_level]
        return output_traces

def hw():
    return np.array([bin(x).count("1") for x in range(256)])

def calculate_HW(data):
    if isinstance(data, int):
        print('Input must be an array')

    if data.ndim == 1:
        return hw()[data]
    else:
        return np.reshape([hw()[data.ravel()]], np.shape(data))

def calculate_HW_2D(data):
    return np.array([calculate_HW(s) for s in data])

# select POI with CPA
def POI_selection(traces, label, numPOIs, POIspacing):
    corr = np.zeros(len(traces[0]))

    for i in range(len(traces[0])):
        if (traces[:, i] == traces[:, i][0]).all():
            corr[i] = 0
        else:
            corr[i] = abs(stats.pearsonr(traces[:, i], label)[0])

    POIs = []
    # Repeat until we have enough POIs
    for i in range(numPOIs):
        # Find the biggest peak and add it to the list of POIs
        nextPOI = corr.argmax()
        POIs.append(nextPOI)
        # Zero out some of the surrounding points
        # Make sure we don't go out of bounds
        poiMin = max(0, nextPOI - POIspacing)
        poiMax = min(nextPOI + POIspacing, len(corr))
        for j in range(poiMin, poiMax):
            corr[j] = 0
    return POIs

def load_ascad(ascad_database_file, target_byte, profiling_traces=50000):
    in_file = h5py.File(ascad_database_file + 'ASCAD_1400_points_sbox{}_desync0.h5'.format(target_byte), "r")
    X_profiling = np.array(in_file['Profiling_traces/traces'], dtype=np.float32)
    X_profiling = X_profiling.reshape((X_profiling.shape[0], X_profiling.shape[1]))
    Y_profiling = np.array(in_file['Profiling_traces/labels'], dtype=np.int16)
    P_profiling = np.array(in_file['Profiling_traces/metadata'][:]['plaintext'], dtype=np.int16)
    
    # Load attack traces
    X_attack = np.array(in_file['Attack_traces/traces'], dtype=np.float32)
    X_attack = X_attack.reshape((X_attack.shape[0], X_attack.shape[1]))
    Y_attack = np.array(in_file['Attack_traces/labels'], dtype=np.int16)
    P_attack = np.array(in_file['Attack_traces/metadata'][:]['plaintext'], dtype=np.int16)

    X_all = np.concatenate((X_profiling, X_attack), axis=0)
    Y_all = np.concatenate((Y_profiling, Y_attack), axis=0)
    plt_all = np.concatenate((P_profiling, P_attack), axis=0)
    return X_all, np.array(Y_all),  plt_all
    
def load_ascad_rand(ascad_database_file, target_byte, profiling_traces=60000):
    in_file = h5py.File(ascad_database_file + 'ASCAD_rand_5k_points_sbox{}_desync0.h5'.format(target_byte), "r")
    X_profiling = np.array(in_file['Profiling_traces/traces'], dtype=np.float32)
    X_profiling = X_profiling.reshape((X_profiling.shape[0], X_profiling.shape[1]))
    Y_profiling = np.array(in_file['Profiling_traces/labels'], dtype=np.int16)
    P_profiling = np.array(in_file['Profiling_traces/metadata'][:]['plaintext'], dtype=np.int16)
    
    # Load attack traces
    X_attack = np.array(in_file['Attack_traces/traces'], dtype=np.float32)
    X_attack = X_attack.reshape((X_attack.shape[0], X_attack.shape[1]))
    Y_attack = np.array(in_file['Attack_traces/labels'], dtype=np.int16)
    P_attack = np.array(in_file['Attack_traces/metadata'][:]['plaintext'], dtype=np.int16)

    X_all = np.concatenate((X_profiling, X_attack), axis=0)
    Y_all = np.concatenate((Y_profiling, Y_attack), axis=0)
    plt_all = np.concatenate((P_profiling, P_attack), axis=0)
    return X_all[:60000], np.array(Y_all[:60000]),  plt_all[:60000]

def load_aeshd(AESHD_data_folder, target_byte, profiling_traces=45000):
    def xor_bytes(a, b):
        return [a[i] ^ b[i] for i in range(len(a))]

    def expand_key(master_key):
        iteration_count = 0
        for i in range(4, 44):
            word = list(master_key[len(master_key) - 4:])

            if i % 4 == 0:
                word.append(word.pop(0))
                word = [AES_Sbox[b] for b in word]
                word[0] ^= r_con[i // 4]

            word = xor_bytes(word, master_key[iteration_count * 4:iteration_count * 4 + 4])
            for w in word:
                master_key.append(w)

            iteration_count += 1

        return [master_key[16 * i: 16 * (i + 1)] for i in range(len(master_key) // 16)]


    def get_round_keys(keys):
        """ Compute round keys for all keys" """

        keys = np.array(keys, dtype='uint8')
        if np.all(keys == keys[0]):
            """ If all keys are equal, then compute round keys for one key only """
            round_keys = expand_key(list(keys[0]))
            return np.full([len(keys), len(round_keys), len(round_keys[0])], round_keys)
        else:
            return [expand_key(list(key)) for key in keys]


    def get_labels_from_output(ciphertexts, keys, byte):
        """
        Labels for Hammind Distance leakage model: HD(InvSbox(c[i] xor k[i]), c[j]) = InvSbox(c[i] xor k[i]) xor c[j]
        k[i] = target key byte i of round key 10
        c[i] = ciphertext i
        c[j] = ciphertext j (j is different from i because of ShiftRows)
        """

        """ Compute round keys """
        round_keys = get_round_keys(keys)
        """ get ciphertext bytes c[i] and c[j]"""
        c_j = [cj[shift_row_mask[byte]] for cj in ciphertexts]
        c_i = [ci[byte] for ci in ciphertexts]
        """ get key byte from round key 10 """
        k_i = [ki[10][byte] for ki in round_keys]
        return [AES_Sbox_inv[int(ci) ^ int(ki)] ^ int(cj) for ci, cj, ki in zip(np.asarray(c_i[:]), np.asarray(c_j[:]), np.asarray(k_i[:]))]
    
    shift_row_mask = np.array([0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11])
    r_con = (
        0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
        0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
        0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
        0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
    )

    in_file = h5py.File(AESHD_data_folder + 'aes_hd.h5', "r")

    X_profiling = np.array(in_file['Profiling_traces/traces'], dtype=np.float64)
    X_attack = np.array(in_file['Attack_traces/traces'], dtype=np.float64)
    profiling_key = in_file['Profiling_traces/metadata']['key']
    attack_key = in_file['Attack_traces/metadata']['key']
    C_profiling = np.array(in_file['Profiling_traces/metadata']['ciphertext'], dtype=np.int16)
    C_attack = np.array(in_file['Attack_traces/metadata']['ciphertext'], dtype=np.int16)
    Y_profiling = np.array(get_labels_from_output(C_profiling, profiling_key, target_byte), dtype=np.int16)
    Y_attack = np.array(get_labels_from_output(C_attack, attack_key, target_byte), dtype=np.int16)
    X_all = np.concatenate((X_profiling, X_attack), axis=0)
    Y_all = np.concatenate((Y_profiling, Y_attack), axis=0)
    plt_all = np.concatenate((C_profiling, C_attack), axis=0)
    return X_all, np.array(Y_all),  plt_all

def load_chesctf(database_file, target_byte, profiling_traces=50000):
    in_file = h5py.File(database_file+'ches_ctf.h5', "r")
    # Load profiling traces
    X_profiling = np.array(in_file['Profiling_traces/traces'], dtype=np.float32)
    X_profiling = X_profiling.reshape((X_profiling.shape[0], X_profiling.shape[1]))
    # Load attacking traces
    X_attack = np.array(in_file['Attack_traces/traces'], dtype=np.float32)
    X_attack = X_attack.reshape((X_attack.shape[0], X_attack.shape[1]))
    plt_profiling = np.array(in_file['Profiling_traces/metadata'][:]['plaintext'], dtype=np.int16)
    plt_attack = np.array(in_file['Attack_traces/metadata'][:]['plaintext'], dtype=np.int16)
    key_profiling = np.array(in_file['Profiling_traces/metadata']['key'], dtype=np.int16)
    key_attack = np.array(in_file['Attack_traces/metadata']['key'], dtype=np.int16)
    # Attack byte 0
    Y_profiling = labelize(plt_profiling, key_profiling, target_byte)
    Y_attack = labelize(plt_attack, key_attack, target_byte)
    print('Profiling traces number: {}'.format(len(X_profiling)))
    print('Attack traces number: {}'.format(len(X_attack)))
    return X_profiling, Y_profiling,  plt_profiling

def load_aesrd(database_file, target_byte, profiling_traces=50000):
    # Load profiling traces
    X_profiling = np.load(database_file+"profiling_traces_AES_RD.npy")
    X_profiling = X_profiling.reshape((X_profiling.shape[0], X_profiling.shape[1]))
    # Load attacking traces
    X_attack = np.load(database_file+"attack_traces_AES_RD.npy")
    X_attack = X_attack.reshape((X_attack.shape[0], X_attack.shape[1]))
    plt_profiling = np.load(database_file+"profiling_plaintext_AES_RD.npy")
    plt_attack = np.load(database_file+"attack_plaintext_AES_RD.npy")
    key = np.load(database_file+"key.npy")

    Y_profiling = labelize(plt_profiling, key, target_byte)
    Y_attack = labelize(plt_attack, key, target_byte)

    X_profiling = np.concatenate((X_profiling, X_attack), axis=0)
    Y_profiling = np.concatenate((Y_profiling, Y_attack), axis=0)
    plt_profiling = np.concatenate((plt_profiling, plt_attack), axis=0)
    print('Profiling traces number: {}'.format(len(X_profiling)))
    return X_profiling, np.array(Y_profiling),  plt_profiling